		...
		<header
			className={cn(
                ...
                open 
                    ? 'bg-[#0e1510]' 
                    : scrolled 
                        ? 'bg-[#0e1510]/80 backdrop-blur-xl shadow-[0_24px_48px_rgba(221,229,220,0.02)]' 
                        : 'bg-transparent'
			)}
		>
			<nav
			...
			>
				...
				<div className="hidden md:flex gap-10 items-center">
					{links.map((link, i) => (
						<Link key={i} className="font-['Plus_Jakarta_Sans'] uppercase tracking-[0.1em] text-xs text-[#dde5dc]/60 font-medium hover:text-[#bbcac6] transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)]" href={link.href}>
							{link.label}
						</Link>
					))}
				</div>
				...
				<div className="flex items-center gap-4">
					<button
						onClick={toggleCart}
						className="hidden md:flex font-['Plus_Jakarta_Sans'] uppercase tracking-[0.1em] text-xs text-[#bbcac6] font-medium items-center gap-2 hover:opacity-70 transition-opacity relative"
					>
						<span>BAG</span>
						<span className="material-symbols-outlined text-lg">local_mall</span>
						{count > 0 && (
							<span className="absolute -top-2 -right-3 bg-[#bbcac6] text-[#0e1510] text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-sm">
								{count}
							</span>
						)}
					</button>
			...
			>
				...
				>
					<div className="flex flex-col gap-8">
						{links.map((link) => (
							<Link
								...
								className="font-['Anton'] text-4xl uppercase tracking-widest text-[#bbcac6] hover:text-[#bbcac6]/60 transition-colors"
								href={link.href}
							>
								{link.label}
							</Link>
					...
					<div className="mt-auto flex flex-col gap-6 pt-12 border-t border-[#bbcac6]/10">
						<p className="font-['Plus_Jakarta_Sans'] text-[10px] tracking-[0.2em] text-[#bbcac6]/40 uppercase">Connect</p>
						<div className="flex gap-6">
							<a href="#" className="font-['Plus_Jakarta_Sans'] text-xs tracking-widest text-[#bbcac6] hover:opacity-70 transition-opacity uppercase">Instagram</a>
							<a href="#" className="font-['Plus_Jakarta_Sans'] text-xs tracking-widest text-[#bbcac6] hover:opacity-70 transition-opacity uppercase">Contact</a>
						</div>
					</div>
				</div>
			</div>
